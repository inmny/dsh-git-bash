...
test("guard normalizes the server token default DACL idempotently", { skip: !CAN_RUN_NATIVE_GUARD }, () => {
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const result = spawnSync(GUARD, ["--normalize-server-dacl"], {
      cwd: process.cwd(),
      encoding: "utf8",
      windowsHide: true,
    });
    ...
    assert.equal(result.signal, null);
    assert.match(result.stdout, /^server-dacl=(?:ok|normalized)\r?\n$/);
  }
});
